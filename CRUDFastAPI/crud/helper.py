...
def _extract_matching_columns_from_schema(
    ...
    columns = []
    temp_prefix = temp_prefix if use_temporary_prefix and temp_prefix is not None else ""
    if schema:
        for field in schema.model_fields.keys():
            if hasattr(model_or_alias, field):
                ...
                if prefix is not None or use_temporary_prefix:
                    column_label = f"{temp_prefix}{prefix}{field}" if prefix else f"{temp_prefix}{field}"
        ...
        for column in model.__table__.c:
            ...
            if prefix is not None or use_temporary_prefix:
                column_label = f"{temp_prefix}{prefix}{column.key}" if prefix else f"{temp_prefix}{column.key}"
                column = column.label(column_label)
            columns.append(column)
...
def _auto_detect_join_condition(base_model: type[DeclarativeBase], join_model: type[DeclarativeBase]) -> Optional[ColumnElement]:
    """
    ...
    if inspector is not None:
        ...
        join_on = next(
            (base_model.__table__.c[col.name] == join_model.__table__.c[list(col.foreign_keys)[0].column.name] for col in fk_columns if list(col.foreign_keys)[0].column.table == join_model.__table__),
            None,
        )
...
def _nest_join_data(
    ...
    for key, value in data.items():
        ...
        for join in join_definitions:
            ...
            if isinstance(key, str) and key.startswith(full_prefix):
                nested_key = join_prefix.rstrip("_") if join_prefix else join.model.__tablename__
                nested_field = key[len(full_prefix) :]

                if join.relationship_type == "one-to-many":
                    nested_data = _handle_one_to_many(nested_data, nested_key, nested_field, value)
                else:
                    nested_data = _handle_one_to_one(nested_data, nested_key, nested_field, value)

                nested = True
                break

        if not nested:
            stripped_key = key[len(temp_prefix) :] if isinstance(key, str) and key.startswith(temp_prefix) else key
    ...
    for join in join_definitions:
        ...
        nested_key = join.join_prefix.rstrip("_") if join.join_prefix else join.model.__tablename__
        if join.relationship_type == "one-to-many" and nested_key in nested_data:
            if isinstance(nested_data.get(nested_key, []), list):
                if any(item[join_primary_key] is None for item in nested_data[nested_key]):
                    nested_data[nested_key] = []
...
def _nest_multi_join_data(
    ...
    Args:
        ...
        schema_to_select: Pydantic schema for selecting specific columns from the primary model. Used for converting dictionaries back to Pydantic models.
                ...
                for prefix, schema in nested_schema_to_select.items():
                    if prefix in item:
                        if isinstance(item[prefix], list):
                            item[prefix] = [schema(**nested_item) for nested_item in item[prefix]]
                        else:  # pragma: no cover
