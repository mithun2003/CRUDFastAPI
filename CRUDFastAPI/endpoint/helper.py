...
class FilterConfig(BaseModel):
    ...
    def check_filter_types(cls, filters: dict[str, Any]) -> dict[str, Any]:
        for key, value in filters.items():
            if not isinstance(value, Union[type(None), str, int, float, bool]):
...
def _get_python_type(column: Column) -> Optional[type]:
    try:
        ...
        if hasattr(column.type, "impl") and hasattr(column.type.impl, "python_type"):
            ...
            raise NotImplementedError(f"The primary key column {column.name} uses a custom type without a defined `python_type` or suitable `impl` fallback.")
...
def _temporary_dependency_handling(
    ...
    if funcs is not None:
        if any(isinstance(func, params.Depends) for func in funcs):
            ...
            return [func if isinstance(func, params.Depends) else Depends(func) for func in funcs]
    return None
...
def _apply_model_pk(**pkeys: dict[str, type]):
    ...
    def wrapper(endpoint):
        ...
        extra_positional_params = [inspect.Parameter(name=k, annotation=v, kind=inspect.Parameter.POSITIONAL_ONLY) for k, v in pkeys.items()]

        endpoint.__signature__ = signature.replace(parameters=extra_positional_params + parameters)
        return endpoint

    return wrapper


def _create_dynamic_filters(filter_config: Optional[FilterConfig], column_types: dict[str, type]) -> Callable[..., dict[str, Any]]:
