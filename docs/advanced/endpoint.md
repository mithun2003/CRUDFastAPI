...
CRUDFastAPI automates the creation of CRUD (Create, Read, Update, Delete) endpoints for your FastAPI application. Here's an overview of the available automatic endpoints and how they work:
...
To implement custom soft delete columns using `EndpointCreator` and `crud_router` in CRUDFastAPI, you need to specify the names of the columns used for indicating deletion status and the deletion timestamp in your model. CRUDFastAPI provides flexibility in handling soft deletes by allowing you to configure these column names directly when setting up CRUD operations or API endpoints.
...
When initializing `crud_router` or creating a custom `EndpointCreator`, you can pass the names of your custom soft delete columns through the `CRUDFastAPI` initialization. This informs CRUDFastAPI which columns to check and update for soft deletion operations.
...
# Initialize CRUDFastAPI with custom soft delete columns
my_model_crud = CRUDFastAPI(MyModel,
                         is_deleted_column='archived',  # Custom 'is_deleted' column name
                         deleted_at_column='archived_at'  # Custom 'deleted_at' column name
                        )

# Setup CRUD router with the CRUDFastAPI instance
app.include_router(crud_router(
    session=async_session,
    model=MyModel,
    crud=my_model_crud,
...
By specifying custom column names for soft deletion, you can adapt CRUDFastAPI to fit the design of your database models, providing a flexible solution for handling deleted records in a way that best suits your application's needs.

## Conclusion

The `EndpointCreator` class in CRUDFastAPI offers flexibility and control over CRUD operations and custom endpoint creation. By extending this class or using the `included_methods` and `deleted_methods` parameters, you can tailor your API's functionality to your specific requirements, ensuring a more customizable and streamlined experience.
